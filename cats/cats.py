...
def about(subject):
    ...
    def f(x):
        l = x.split()
        l = [lower(y) for y in l]
        l = list(map(remove_punctuation, l))
        
        return any([y in l for y in subject])
    return f
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    dif_list = [diff_function(typed_word, x, limit) for x in word_list]
    if typed_word in word_list:
        return typed_word
    elif min(dif_list) > limit:
        return typed_word
    else:
        return word_list[dif_list.index(min(dif_list))]
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if length == 0 and length_diff != 0:
        return length_diff
    if length == 0:
        return 0
    else:
        return f(typed[0], source[0]) + feline_fixes(typed[1:], source[1:], limit)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, upload):
    ...
    progress = 0
    for word in typed:
        if word != prompt[typed.index(word)]:
            break
        progress += 1
...
def time_per_word(words, times_per_player):
    ...
    l_0 = []
    l_1 = []
    for i in range(1, len(times_per_player[0])):
        l_0.append(times_per_player[0][i+1] - times_per_player[0][i])
        l_1.append(times_per_player[1][i+1] - times_per_player[1][i])

    return match(words, [l_0, l_1])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        l[time_list.index(min_time)].append(get_word(match, i))
    return l
